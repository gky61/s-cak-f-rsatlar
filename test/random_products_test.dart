...
import 'file:///d:/firsatkolik/lib/services/category_detection_service.dart';

void main() {
  group('Tricky Products Classification Tests (20 Products)', () {
    ...
    test('Analyze classifications of 20 tricky products', () {
      for (final product in testProducts) {
        final String name = product['name'];
        final String expectedCat = product['expectedCategory'];
        final String expectedSubCat = product['expectedSubCategory'];
        final String title = product['title'];
        final List<String> breadcrumbs = List<String>.from(product['breadcrumbs']);

        print('\n----------------------------------------');
        print('Testing product: $name');
        print('Title: "$title"');
        print('Breadcrumbs: $breadcrumbs');

        final textToClassify = '${breadcrumbs.join(' ')} $title';
        final result = CategoryDetectionService.detectCategory(textToClassify);

        print('Result: $result');

        expect(result, isNotNull, reason: 'Category detection should not return null for $name');
        expect(result!['categoryId'], expectedCat, reason: 'Category ID mismatch for $name');
        expect(result['subCategory'], expectedSubCat, reason: 'SubCategory mismatch for $name');
        
        print('🎉 SUCCESS: Correctly mapped to $expectedCat -> $expectedSubCat');
      }
    });
  });
}
