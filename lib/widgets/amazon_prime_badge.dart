...
/// Amazon "Prime Fırsatı" özel fırsat rozeti (Mor-Turuncu Minimalist Tasarım)
class AmazonPrimeBadge extends StatelessWidget {
  final bool compact;
  ...
  /// Yardımcı metot: Fırsat bir Amazon Prime fırsatı mı?
  static bool isAmazonPrimeDeal(dynamic deal) {
    if (deal == null) return false;
    final store = (deal.store ?? '').toString().toLowerCase();
    if (!store.contains('amazon') && !store.contains('amzn')) return false;

    // priceLabel içinde PRIME bulunması
    final hasPrimeLabel = deal.priceLabel != null &&
        deal.priceLabel.toString().toUpperCase().contains('PRIME');

    return hasPrimeLabel;
  }
}
