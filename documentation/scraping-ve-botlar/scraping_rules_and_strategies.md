...
*   **Canlı Fiyat API Entegrasyonu & `_HbApiPriceResult` Modeli:** Hepsiburada normal, Premium ve sepetteki indirimli fiyatları HTML dokümanında gizler. Scraper:
    ...
    3. **Etiket Filtreleme (`_isIgnoredHbTag`):** `product.tagList` ve `otherMerchants` listesindeki `premium-a-gec`, `premiuma-gec`, `ilk-siparis`, `yeni-uye` gibi genel üyelik/onboarding upsell etiketleri elenir; böylece API'nin sahte 100 TL'lik "Premium'a Geç 100 TL İndirim Kazan" promosyonunu ürün indirimi sanması engellenir.
    ...
    6. **Premium vs Normal Sepet İndirimi Ayrıştırması (`_isValidPremiumCampaignResult`):** Gelen API yanıtındaki `evaluateAsPremiumResult` nesnesi doğrulanır (onboarding promosyonları elenir). Gerçek Premium kampanyası (`isPremium: true`) ile normal sepete özel indirimler (`evaluateResult` / `priceData`, `isPremium: false`) karşılaştırılır. Fiyatlar eşitse Premium avantajı önceliklendirilir, normal sepete özel indirimlerde ise `isPremium: false` korunarak en avantajlı sonuç `_lastApiResult` referansına kaydedilir.
